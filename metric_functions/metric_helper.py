...
def llr(ref_wav, deg_wav, srate):
    ...
    for frame_count in range(num_frames):
        ...
        # Skipping silent frame
        if np.all(clean_frame == 0) or np.all(processed_frame == 0):
            # print("Skipping silent frame")
        ...
        if denominator > 0:
            ...
            # print(f"Warning: Non-positive denominator encountered. Skipping frame {frame_count}.")
            distortion.append(np.nan)
