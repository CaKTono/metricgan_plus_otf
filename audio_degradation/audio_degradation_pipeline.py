...
import math
from metric_functions.get_metric_scores import get_pesq_score, get_csig_score, get_cbak_score, get_covl_score
...
def read_audio(filename, force_1ch=False, fs=None):
    try:
        # print(f"Reading audio file: {filename}")  # Debug statement
...
def check_scores(clean_sample, noisy_sample):
    try:
        clean_sample = clean_sample.astype(np.float32).squeeze()
        noisy_sample = noisy_sample.astype(np.float32).squeeze()

        # Use in-memory scoring functions
        pesq_score = get_pesq_score(clean_sample, noisy_sample, norm=True)
        csig_score = get_csig_score(clean_sample, noisy_sample, norm=True)
        cbak_score = get_cbak_score(clean_sample, noisy_sample, norm=True)
        covl_score = get_covl_score(clean_sample, noisy_sample, norm=True)

        if pesq_score > 0 and csig_score > 0 and cbak_score > 0 and covl_score > 0:
            return True
        else:
            return False
    except Exception as e:
...
def process_from_audio_path(
    ...
    length=None,
    check_scores_before_return=False  # New parameter to control score checking, True for checking and False to skip
):
    while True:  # Loop until valid scores are obtained or not checking scores
        try:
            if fs is None:
            ...
            if check_scores_before_return:
                if check_scores(vocal, noisy_vocal):
                    print("process_from_audio_path completed with valid scores")
                    return np.asarray(vocal), np.asarray(noise), np.asarray(noisy_vocal), fs
                else:
                    print("Invalid scores, retrying...")
                    continue
            else:
                # print("process_from_audio_path completed without score checking")
                return np.asarray(vocal), np.asarray(noise), np.asarray(noisy_vocal), fs

        except Exception as e:
            # print(f"Error processing audio paths: {vocal_path}, {noise_path}, {rir_path}: {e}")
            raise
