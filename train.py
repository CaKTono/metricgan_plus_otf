...
Reimplemented: Calvin Kristianto Thayono
 - Adjusted to handle CUDA_VISIBLE_DEVICES
 - Retry From latest epoch if the training has an error
...
from metric_functions.get_metric_scores_path import get_pesq_parallel, get_csig_parallel, get_cbak_parallel, get_covl_parallel
...
class Trainer:
    ...
    def init_model_optim(self):
        """
        Add nn.DataParallel if there are multiple GPUs
        """
    ...
    def load_checkpoint(self, ver='latest'):
        ...
        if ver == 'best':
            ...
            print(f'---load latest model weights and optimizer---')
        ...
        while self.epoch <= self.args.epochs:
            try:
                for epoch in np.arange(self.epoch, self.args.epochs + 1):
                    ...
                    random.shuffle(self.generator_train_paths)
                    genloader = create_dataloader(self.generator_train_paths[0:round(1 * self.args.num_of_sampling)], 
                                                  self.train_noisy_path, num_target_metric=1, loader='G', 
                                                  batch_size=self.args.batch_size, num_workers=self.args.num_workers)
                    self.train_one_epoch(genloader)

                break  # Break the loop if training completes successfully

            except Exception as e:
                print(f"Error occurred during training: {e}. Retrying epoch {self.epoch}...")
                # Retry the current epoch
                continue  # This will restart the current epoch loop
    ...
    def get_score(self):
        ...
        with torch.no_grad():
            for path in tqdm(D_paths):
                ...
                clean_wav, _ = torchaudio.load(path)
                noise_wav, _ = torchaudio.load(os.path.join(self.train_noisy_path, wave_name))
                noise_mag, noise_phase = get_spec_and_phase(noise_wav.to(self.device))
                ...
                Noised_name.append(os.path.join(self.train_noisy_path, wave_name))
